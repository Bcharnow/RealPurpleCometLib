...
namespace v3 {
	...
	inline Vector3 YawPitch(float yaw, float pitch) {
		...
		dir.x = cos(toRadians(yaw)) * cos(toRadians(pitch));
		dir.y = sin(toRadians(pitch));
		dir.z = sin(toRadians(yaw)) * cos(toRadians(pitch));
		return normal(dir);
	}
	...
	{
		bool c1 = apxequal(p.x, p1.x);
		bool c2 = apxequal(p.y, p1.y);
		bool c3 = apxequal(p.z, p1.z);
